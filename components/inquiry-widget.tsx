...
export function InquiryWidget() {
  ...
  useEffect(() => {
    if (open) composer.current?.querySelector<HTMLElement>("input:not([type=hidden]), textarea, button, a")?.focus({ preventScroll: true });
    else if (initialized.current) launcher.current?.focus();
  }, [open, step, choosing]);
  useEffect(() => {
    if (open && transcript.current) transcript.current.scrollTop = transcript.current.scrollHeight;
  }, [open, messages, error, pending]);
  useEffect(() => {
  ...
  function go(next: Step, reply?: string, data = answers) {
    ...
    setMessages(previous => [...previous,
      ...(reply ? [{ role: "visitor" as const, text: reply }] : []),
      { role: "assistant", text: question(next, data), ...(next === "review" ? { summary: { first_name: data.first_name, last_name: data.last_name, email: data.email, phone: data.phone, preferred_therapist: data.preferred_therapist } } : {}) },
    ]);
  }
  ...
  return (
    <aside ref={widget} className={styles.widget} data-open={open} aria-label="Contact inquiry assistant">
      {prompt && !open && <div className={styles.prompt}>
        ...
        <p><strong>{greeting}</strong></p><p>{welcome}</p><p className={styles.privacy}>{privacy}</p>
        <button className={styles.primary} onClick={expand}>Start</button>
      </div>}
      {open && <section id="inquiry-widget-panel" role="dialog" aria-labelledby="widget-title" className={styles.panel}
        onKeyDown={event => { if (event.key === "Escape") { event.stopPropagation(); close(); } }}>
        <header className={styles.header}>
          <span className={styles.mark} aria-hidden="true"><svg viewBox="0 0 24 24" width="23" height="23" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M20 11.5a8 8 0 0 1-8 8H5l-4 3 1.5-6A8 8 0 1 1 20 11.5Z"/><path d="M6 10h8M6 14h5"/></svg></span>
        ...
        <div ref={transcript} className={styles.transcript} role="log" aria-label="Inquiry conversation" aria-live="polite" aria-relevant="additions" aria-atomic="false" tabIndex={0}>
          ...
          {pending && <p className={styles.assistant}>Sending your inquiry…</p>}
          {error && step === "review" && <div className={styles.assistant}><p>{error}</p><p><a href="tel:9032838729">(903) 283-8729</a> or <a href="mailto:info@thebridgetherapy.com">info@thebridgetherapy.com</a></p></div>}
        </div>
        <div ref={composer} className={styles.composer}>
          ...
          {step === "preferred_therapist" && <div className={styles.choices} aria-label="Therapist choices">
            {profileChoice ? <>
              ...
              <button className={styles.chip} onClick={() => { setChoosing(true); setMessages(previous => [...previous, { role: "visitor", text: "Help me choose someone else" }, { role: "assistant", text: "Who would you like to ask about?" }]); }}>Help me choose someone else</button>
